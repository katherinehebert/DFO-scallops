...
# Load Greenland halibut 4RST catch and spawning biomass index time series (scallop.landing.index.csv) from data folder on the github repository https://github.com/MathBoud/C68/data 
scallop <- readRDS("data/cpue-landings-yearly.rds")

plot.Landings <- ggplot2::ggplot(scallop) +
...
# load function, because package won't install from github:
source('scripts/assess_SSS.R')
source('~/Documents/GitHub/DFO-scallops/MSEtool-master/R/Misc_fun.R')
...
DLMobject@Common_Name <- 'Scallop'        #  Common name of the species

DLMobject@Cat <- matrix((scallop$Land.tons) ,nrow=1) #  Total annual catches (Matrix of nsim rows and nyears columns)
...
DLMobject@Year <- as.integer(scallop$Year)             #  Years of the catch and abundance index time series
DLMobject@Units <- "tonnes"                         #  State units of catch

# unchanged from MathBoud's script
...
Landing.Index.data <- left_join(scallop, Biomass, by="Year")
...
Biomass.plot<-ggplot() +
  ...
  ylim(0,85000) +
  scale_x_continuous(breaks = seq(from=min(Biomass$Year, na.rm = TRUE), to=max(Biomass$Year, na.rm = TRUE), by = 5)) +
  ggtitle("4RST-scallop Stock biomass") +
  theme_classic() +  theme(axis.title = element_text(size = 16, colour = "black"),
                           axis.text = element_text(size = 12),
                           plot.title = element_text(size = 15, colour = "blue3", face = "bold"),
                           strip.text.x = element_text(size = 14, face="bold"))
